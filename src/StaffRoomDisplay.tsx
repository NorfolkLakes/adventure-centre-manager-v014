...
export default function StaffRoomDisplay({ onExit }: { onExit: () => void }) {
  ...
  async function load() {
    const [{ data: live }, { data: off }] = await Promise.all([
      ...
      supabase.from('staff_days_off').select('id,staff_id,staff_email,staff_name,day,status,note').eq('day', isoDate(new Date())),
    ])
  ...
  const offByStaff = useMemo(() => new Map(daysOff.map(item => [item.staff_id,item])), [daysOff])
  const visibleStaff = useMemo(() => [...state.staff].sort((a,b) => a.name.localeCompare(b.name)), [state.staff])
  const offGroups = useMemo(() => daysOff.reduce<Record<string,StaffDayOff[]>>((acc,item) => { (acc[item.status] ??= []).push(item); return acc },{}), [daysOff])
  ...
  return <main className="staff-display-shell">
    ...
    {!state.programme || !programmeDay ? <section className="staff-display-empty"><Users size={42}/><h2>No programme is live for today</h2><p>The display will update automatically when a programme and staffing are published.</p></section> : <>
      <section className="staff-display-summary"><article><span>Programme</span><strong>{state.programme.title}</strong></article><article><span>Staff shown</span><strong>{visibleStaff.length}</strong></article><article><span>Off today</span><strong>{daysOff.length}</strong></article><article><span>Last updated</span><strong>{updatedAt ? new Date(updatedAt).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'}) : '—'}</strong></article></section>
      <div className="staff-display-layout">
        ...
        <aside className="staff-display-off-panel"><h2>Days off today</h2>{(['hol','sick','off','am_off','pm_off'] as DayOffStatus[]).map(status => <section key={status} className={`off-list off-${status}`}><h3>{statusLabel(status)}</h3>{offGroups[status]?.length ? offGroups[status].map(item => <div key={item.id}><strong>{item.staff_name}</strong>{item.note && <small>{item.note}</small>}</div>) : <p>None</p>}</section>)}</aside>
      </div>
    </>}
