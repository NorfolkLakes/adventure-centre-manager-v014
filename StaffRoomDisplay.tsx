...
export default function StaffRoomDisplay({ onExit }: { onExit: () => void }) {
  ...
  const upcomingHolidays = useMemo(() => daysOff.filter(item => item.status === 'hol' && item.day > todayIso).sort((a,b) => a.day.localeCompare(b.day)), [daysOff, todayIso])
  ...
  return <main className="staff-display-shell">
    ...
    {!state.programme || !programmeDay ? <section className="staff-display-empty"><Users size={42}/><h2>No programme is live for today</h2><p>The display will update automatically when a programme and staffing are published.</p></section> : <>
      ...
      <div className="staff-display-layout">
        ...
        <aside className="staff-display-off-panel"><h2>Days off</h2>{(['hol','sick','off','am_off','pm_off'] as DayOffStatus[]).map(status => <section key={status} className={`off-list off-${status}`}><h3>{status === 'hol' ? 'HOLIDAY TODAY' : statusLabel(status)}</h3>{offGroups[status]?.length ? offGroups[status].map(item => <div key={item.id}><strong>{item.staff_name}</strong>{item.note && <small>{item.note}</small>}</div>) : <p>None</p>}</section>)}<section className="off-list upcoming-holiday"><h3>UPCOMING HOLIDAY · NEXT 7 DAYS</h3>{upcomingHolidays.length ? upcomingHolidays.map(item => <div key={`${item.id}-${item.day}`}><strong>{new Date(`${item.day}T12:00:00`).toLocaleDateString('en-GB',{weekday:'short',day:'numeric',month:'short'})} — {item.staff_name}</strong>{item.note && <small>{item.note}</small>}</div>) : <p>None</p>}</section></aside>
      </div>
    </>}
